...
public class ClientDAO extends DAO {
	...
	public Client getById(int idClient) {
		String sql = "SELECT c FROM Client c WHERE c.idClient = "
	...
	public int add(Client client) {
		...
		Query query = entityManager
				.createNativeQuery("BEGIN package_client.insertion_client(?,null,null,null,?, "
	...
	public int login(String email, String password) {
		...
		Query query = entityManager
				.createNativeQuery("SELECT PACKAGE_CLIENT.CONNEXION(?,?) AS id FROM DUAL");
		
		query.setParameter(1, email);
		query.setParameter(2, password);
		
		int id = ((BigDecimal)query.getSingleResult()).intValueExact();
